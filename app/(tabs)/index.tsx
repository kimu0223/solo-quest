import { analyzeVoiceReport } from '@/lib/gemini'; // ★追加：AI関数
import { supabase } from '@/lib/supabase';
import { Session } from '@supabase/supabase-js';
import { Audio } from 'expo-av';
import * as FileSystem from 'expo-file-system'; // ★追加：ファイル読み込み用
...
export default function App() {
  ...
  const [isAnalyzing, setIsAnalyzing] = useState(false); // ★追加：鑑定中フラグ
  ...
  async function stopRecording() {
    ...
    if (uri) {
      try {
        // 1. ファイルをBase64文字列として読み込む
        const base64Audio = await FileSystem.readAsStringAsync(uri, {
          encoding: FileSystem.EncodingType.Base64,
        });

        // 2. Geminiに送信！（クエスト名は仮で「日々の冒険」としています）
        const result = await analyzeVoiceReport(base64Audio, "日々の冒険");

        // 3. 結果を表示
        Alert.alert(
          `鑑定結果：ランク ${result.rank}`,
          `「${result.transcript}」\n\n賢者: ${result.comment}`
        );
        
        // ※ここでXP加算などをすると完璧ですが、それは次のステップで！

      } catch (error) {
        Alert.alert("エラー", "AI鑑定に失敗しました");
        console.error(error);
      }
    }
  ...
  async function completeQuest(questId: string, xpReward: number) {
    ...
    if (updatedPlayer) {
      ...
      if (calcLevel > currentLevel) {
        Alert.alert("🎉 LEVEL UP! 🎉", `${player.name} は Lv.${calcLevel} になった！`);
      } else {
  ...
  function checkGateAnswer() {
    if (gateAnswer === '21') {
      ...
      setGateAnswer('');
      Alert.alert("ギルドマスター認証", "管理者モードになりました。");
    } else {
      Alert.alert("ブブー！", "計算が違います。");
      setGateAnswer('');
    }
  }
      ...
      <View style={styles.recordingArea}>
        ...
        {isAnalyzing ? (
          <View style={{alignItems:'center'}}>
            <ActivityIndicator size="small" color={player.mana_color} />
            <Text style={{color:'#ccc', marginTop:10}}>賢者が鑑定中...</Text>
          </View>
        ) : (
          <TouchableOpacity
          ...
          >
            <Text style={styles.recordButtonText}>{recording ? '■ 報告を終える' : '● 報告する'}</Text>
          </TouchableOpacity>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  recordingArea: { marginBottom: 20, alignItems: 'center', padding: 20, backgroundColor: '#1e1e1e', borderRadius: 15, minHeight: 100, justifyContent: 'center' },
